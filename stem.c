/* Based on http://tartarus.org/~martin/PorterStemmer/c_thread_safe.txt */
...
int stem_sub(struct stemmer * z, char * b, int k)
{
...
extern char* stem(char* s) {
  struct stemmer* z = create_stemmer();
  s[stem_sub(z,s,strlen(s)-1)+1] = 0;
  free_stemmer(z);
  return s;
}
